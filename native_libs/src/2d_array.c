...
char **join(char **fst_mat, char **snd_mat, size_t fst_rows, size_t fst_cols, size_t snd_rows, size_t snd_cols){
    size_t cols = ((fst_cols > snd_cols) ? fst_cols : snd_cols);
    char **new_mat = malloc((fst_rows+snd_rows)*cols*sizeof(*fst_mat));
