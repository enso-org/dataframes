...
instance Progress.Progress UnpackProgressInfo where
    ratio p = Just $ fromIntegral (percentProgress p) / 100
...
parseProgress chunk = eitherToMaybe $ parseChunk where
...
retrieveProgressFromOutput text = infos where
    -- Note: progress updates are not real lines, they are separated only by '\r'
...
instance Exception UnpackingException where
    displayException UnpackingException{..} = printf "failed to unpack %s to %s: process returned code %d, stderr: %s" _archivePath _outputDirectory _stderr _exitCode
...
unpackWithProgress :: (MonadMask m, MonadThrow m, MonadIO m) 
                   ...
                   -> FilePath  -- ^ Output directory
                   -> m ()
unpackWithProgress cb archivePath outputDirectory = do
    let action = unpackWithProgress' archivePath outputDirectory
    Progress.runProgressible action cb
...
unpackWithProgress' archivePath outputDirectory cb = do
    ...
    let outConduit = decodeUtf8LenientC .| mapM_C (processOutputChunk cb)
    let errConduit = decodeUtf8LenientC .| foldC
    (result, out, err) <- liftIO $ sourceProcessWithStreams proc inputConduit outConduit errConduit
    case result of
        ExitSuccess -> pure ()
